...
console.log("Electron's V8 version: ", process.versions.v8);
...
try {
  ...
  console.log('LOADED TFJS-NODE Successfully');

} catch (error) {
  log.info('FAILED TO LOAD TFJS-NODE:', error);
  log.info('Error stack:', error.stack);
  console.error('FAILED TO LOAD TFJS-NODE:', error);
}
...
app.on('window-all-closed', () => {
  if (process.platform !== 'darwin') {
       // Only quit if the main window is also closed: allow indep. closure of stats window
       if (!win) {
        app.quit();
    }  
  }
...
ipcMain.handle('apply_classifier', async (event, images) => {
  ...
  console.log("4) Backend::apply_classifier handler called");
  ...
  try {
      ...
      console.log("sending tf to classify helper: ", tf);
      const predictions = await classify(images, model, tf); // beware of passing null model
...
ipcMain.handle('get_label_colours', async () => {
  console.log("2) Backend::Returning label colors");
